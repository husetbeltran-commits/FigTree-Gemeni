...
const BlessingsPage: React.FC = () => {
  ...
  const categories = Array.from(new Set(BLESSINGS.flatMap(b => b.categories)));
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid gap-3">
          {filtered.map(blessing => (
            <Link key={blessing.id} to={`/blessings/${blessing.id}`} className="bg-surface p-5 rounded-xl border border-border hover:border-accent/30 transition-colors">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="font-bold text-lg mb-1 text-primary">{blessing.title}</h3>
                  {blessing.subtitle && (
                    <p className="text-xs text-secondary mb-2 leading-snug">{blessing.subtitle}</p>
                  )}
                </div>
